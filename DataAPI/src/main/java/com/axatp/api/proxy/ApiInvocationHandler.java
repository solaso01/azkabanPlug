...
import com.axatp.api.model.BaseResponse;
...
public class ApiInvocationHandler implements InvocationHandler {
     ...
     * api 接口实现类实例
     */
    private AzkabanServiceImpl azkabanService;
    /**
     * 重试次数
     */
    ...
    public ApiInvocationHandler(AzkabanServiceImpl azkabanApi) {
        this.azkabanService = azkabanApi;
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        ...
        try {
            while (tryTime <= RETRY) {
                // 指定代理方法
                result = method.invoke(azkabanService, args);
                // 判断是否执行成功
                if (Objects.nonNull(result) && !this.isDefault(method.getName())) {
                    ...
                    if (BaseResponse.SUCCESS.equals(field.get(result))) {
                        log.info("Execute the com.axatp.api.serice's API {} successfully.", method.getName());
                        return result;
                    }
                    azkabanService.login();
                }
